...
int main(void) {
    ...
    /* Create inter-task queues BEFORE creating tasks */
...
PRIORITY_HEALTH, NULL) == pdPASS);
    configASSERT(xTaskCreate(command_handler_task, "CMD" , STACK_COMMAND, NULL,
PRIORITY_COMMAND, NULL) == pdPASS);

    
    uart_puts("Tasks created. Starting scheduler...\r\n");
    vTaskStartScheduler();  /* Never returns */

    uart_puts("FATEL: Scheduler failed!\r\n");
    for (;;) {}
}

/* ---- Required FreeRTOS hook function ---- */

void vApplicationIdleHook(void) {
    /* Low-power sleep on real hardware: _WFI(); */
    /* Leave empty for QEMU simulation           */
}
